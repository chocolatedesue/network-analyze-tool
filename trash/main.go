...
type NetemConvergenceMonitor struct {
	...
	totalRouteEvents         int64  // 使用原子操作
	totalNetemTriggers       int64  // 使用原子操作
	totalRouteTriggers       int64  // 使用原子操作
...
func logStructuredDataAsync(logger *logrus.Logger, data map[string]interface{}) {
	// 使用select with default来避免阻塞，如果日志队列满了就丢弃
	select {
	case logQueue <- logEntry{logger: logger, data: data}:
		// 成功放入队列
	default:
		// 队列满了，记录到控制台
		fmt.Printf("⚠️  日志队列满，丢弃一条日志\n")
	}
}
...
// 全局日志队列
var logQueue = make(chan logEntry, 100) // 缓冲100条日志
	...
	go func() {
		for entry := range logQueue {
			if jsonData, err := json.Marshal(entry.data); err == nil {
				entry.logger.Info(string(jsonData))
			} else {
...
// cleanupOldEvents 清理过期的事件缓存
func (ncm *NetemConvergenceMonitor) cleanupOldEvents() {
	currentTime := time.Now().UnixMilli()
	...
	ncm.recentQdiscEvents = validEvents
}
...
func (ncm *NetemConvergenceMonitor) handleTriggerEvent(timestamp int64, eventType string, triggerInfo map[string]interface{}, triggerSource string) {
	...
	if triggerSource == "netem" {
		...
		atomic.AddInt64(&ncm.totalRouteTriggers, 1)
	}
...
func (ncm *NetemConvergenceMonitor) handleQdiscEventFromTC(obj *tc.Object, eventType string) {
	...
	if ncm.isNetemRelatedEvent(qdiscInfo, eventType) {
		...
		isMonitoring := state == "MONITORING" && currentSession != nil && !currentSession.IsConverged
		...
		if isMonitoring {
			...
			routeEventData := map[string]interface{}{
				...
				"route_event_number":     totalRouteEvents,
...
func (ncm *NetemConvergenceMonitor) handleRouteEvent(timestamp int64, eventType string, routeInfo map[string]interface{}) {
	...
	if (eventType == "路由添加" || eventType == "路由删除") && state == "IDLE" {
		// 作为触发事件处理
	...
	// 普通路由事件处理 - 先读锁检查，再快速写锁操作
	ncm.sessionMu.RLock()
	currentSession := ncm.currentSession
	state = ncm.state
	ncm.sessionMu.RUnlock()

	if state != "MONITORING" || currentSession == nil {
...
func (ncm *NetemConvergenceMonitor) convergenceChecker(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			...
			// 对会话的收敛检查不需要sessionMu锁，因为CheckConvergence内部有自己的锁
