...
def create_heatmap(data: np.ndarray, config: Dict[str, str], ax, grid_size: Tuple[int, int]) -> None:
    ...
    styling_functions = [
        lambda: ax.set_title(config['title'], fontsize=14, pad=10, fontweight='bold'),
        lambda: ax.set_xlabel('Column', fontsize=11),
        lambda: ax.set_ylabel('Row', fontsize=11),
        lambda: ax.set_xticklabels(range(cols), fontsize=6),
        lambda: ax.set_yticklabels(range(rows), fontsize=6),
...
def plot_outage_analysis_heatmaps(file_path: str, output_path: Optional[str] = None,
                                 grid_size: Tuple[int, int] = (15, 15)) -> None:
    """
    ...
    fig.suptitle(f'Network Outage Anomaly Analysis - 15×15 {topology_type} Topology',
...
def plot_single_outage_metric_heatmap(file_path: str, metric: str,
    ...
    # Apply styling
    rows, cols = grid_size
    ax.set_title(f'Network Outage Analysis: {config["title"]} - 15×15 {topology_type} Topology',
    ...
    plt.figtext(0.5, 0.02,
               f'Each cell represents a router position in 15×15 {topology_type.lower()} topology. Gray cells indicate no data.',
...
def main() -> None:
    ...
    default_behavior = lambda: (
        print("Usage: python fping_outage_draw_15x15.py <csv_file_path> <output_image_path>"),
