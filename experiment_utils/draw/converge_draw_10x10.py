...
def plot_convergence_metrics_heatmaps(file_path: str, output_path: str = None, grid_size: tuple = (10, 10)):
    """
    ...
    fig.suptitle(f'Network Convergence Analysis - 10×10 {topology_type} Topology', fontsize=18, y=0.95, fontweight='bold')
    ...
    for idx, (metric, config) in enumerate(metric_configs.items()):
        ...
        sns.heatmap(
            ...
            ax=ax,
            square=True,
            annot_kws={'size': 5, 'weight': 'bold'}  # Smaller font for 10x10
        )
        
        # Set title and labels
        ax.set_title(config['title'], fontsize=12, pad=8, fontweight='bold')
        ax.set_xlabel('Column', fontsize=10)
        ax.set_ylabel('Row', fontsize=10)

        # Set axis ticks
        ax.set_xticklabels(range(grid_size[1]), fontsize=7)
        ax.set_yticklabels(range(grid_size[0]), fontsize=7)
...
def plot_single_convergence_metric_heatmap(file_path: str, metric: str, output_path: str = None, grid_size: tuple = (10, 10)):
    """
    ...
    # Set title and labels
    ax.set_title(f'Network Convergence Analysis: {config["title"]} - 10×10 {topology_type} Topology', fontsize=16, pad=15, fontweight='bold')
    ax.set_xlabel('Column', fontsize=12)
    ax.set_ylabel('Row', fontsize=12)

    # Add subtitle with explanation
    plt.figtext(0.5, 0.02, f'Each cell represents a router position in 10×10 {topology_type.lower()} topology. Gray cells indicate no data.',
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3:
        ...
        print("Usage: python converge_draw_10x10.py <csv_file_path> <output_image_path>")
