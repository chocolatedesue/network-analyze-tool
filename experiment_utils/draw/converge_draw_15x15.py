...
def plot_convergence_metrics_heatmaps(file_path: str, output_path: str = None, grid_size: tuple = (15, 15)):
    """
    ...
    fig.suptitle(f'Network Convergence Analysis - 15×15 {topology_type} Topology', fontsize=20, y=0.95, fontweight='bold')
    ...
    for idx, (metric, config) in enumerate(metric_configs.items()):
        ...
        sns.heatmap(
            ...
            ax=ax,
            square=True,
            annot_kws={'size': 4, 'weight': 'bold'}  # Smaller font for 15x15
        )
        
        # Set title and labels
        ax.set_title(config['title'], fontsize=14, pad=10, fontweight='bold')
        ax.set_xlabel('Column', fontsize=11)
        ax.set_ylabel('Row', fontsize=11)

        # Set axis ticks
        ax.set_xticklabels(range(grid_size[1]), fontsize=6)
        ax.set_yticklabels(range(grid_size[0]), fontsize=6)
...
def plot_single_convergence_metric_heatmap(file_path: str, metric: str, output_path: str = None, grid_size: tuple = (15, 15)):
    """
    ...
    ax = sns.heatmap(
        ...
        cbar_kws={'label': config['cbar_label'], 'shrink': 0.7},
        square=True,
        annot_kws={'size': 4, 'weight': 'bold'}  # Small font for 15x15
    )

    # Set title and labels
    ax.set_title(f'Network Convergence Analysis: {config["title"]} - 15×15 {topology_type} Topology', fontsize=18, pad=15, fontweight='bold')
    ax.set_xlabel('Column', fontsize=13)
    ax.set_ylabel('Row', fontsize=13)

    # Add subtitle with explanation
    plt.figtext(0.5, 0.02, f'Each cell represents a router position in 15×15 {topology_type.lower()} topology. Gray cells indicate no data.',
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3:
        ...
        print("Usage: python converge_draw_15x15.py <csv_file_path> <output_image_path>")
