...
def create_heatmap(data: np.ndarray, config: Dict[str, str], ax, grid_size: Tuple[int, int]) -> None:
    ...
    heatmap_params = {
        ...
        'ax': ax,
        'square': True,
        'annot_kws': {'size': 7, 'weight': 'bold'}  # Adjusted for 6x6
    }
...
def plot_outage_analysis_heatmaps(file_path: str, output_path: Optional[str] = None,
                                 grid_size: Tuple[int, int] = (6, 6)) -> None:
    """
    ...
    fig.suptitle(f'Network Outage Anomaly Analysis - 6×6 {topology_type} Topology',
...
def plot_single_outage_metric_heatmap(file_path: str, metric: str,
    ...
    # Apply styling
    rows, cols = grid_size
    ax.set_title(f'Network Outage Analysis: {config["title"]} - 6×6 {topology_type} Topology',
    ...
    plt.figtext(0.5, 0.02,
               f'Each cell represents a router position in 6×6 {topology_type.lower()} topology. Gray cells indicate no data.',
...
def main() -> None:
    ...
    default_behavior = lambda: (
        print("Usage: python fping_outage_draw_6x6.py <csv_file_path> <output_image_path>"),
