...
def create_heatmap(data: np.ndarray, config: Dict[str, str], ax, grid_size: Tuple[int, int]) -> None:
    ...
    # Create heatmap with functional configuration optimized for 20x20
    heatmap_params = {
        'annot': True,
        'fmt': config['fmt'],
        'cmap': cmap,
...
def plot_outage_analysis_heatmaps(file_path: str, output_path: Optional[str] = None,
    ...
    """
    Plot anomaly-focused outage analysis heatmaps using functional composition optimized for 20x20.
    ...
    fig.suptitle(f'Network Outage Anomaly Analysis - 20×20 {topology_type} Topology',
...
def plot_single_outage_metric_heatmap(file_path: str, metric: str,
                                     ...
                                     grid_size: Tuple[int, int] = (20, 20)) -> None:
    """
    Plot a single outage metric heatmap using functional approach optimized for 20x20.
    ...
    # Apply styling
    rows, cols = grid_size
    ax.set_title(f'Network Outage Analysis: {config["title"]} - 20×20 {topology_type} Topology',
                fontsize=18, pad=15, fontweight='bold')
    ax.set_xlabel('Column', fontsize=14)
    ax.set_ylabel('Row', fontsize=14)
    ax.set_xticklabels(range(cols), fontsize=6)  # Smaller font for 20x20
    ax.set_yticklabels(range(rows), fontsize=6)  # Smaller font for 20x20
    plt.yticks(rotation=0)

    # Add subtitle
    plt.figtext(0.5, 0.02,
               f'Each cell represents a router position in 20×20 {topology_type.lower()} topology. Gray cells indicate no data.',
...
def main() -> None:
    ...
    default_behavior = lambda: (
        print("Usage: python fping_outage_draw_20x20.py <csv_file_path> <output_image_path>"),
