...
"""
OSPFv3 Torus 拓扑生成器
支持生成任意规模的方形 Torus 网络拓扑配置，使用 OSPFv3 路由协议
支持最大 4000 节点 (约 63x63)，支持单区域和多区域配置

Torus 拓扑特点：
- 边缘节点环绕连接（与 Grid 的主要区别）  
- 所有节点都有4个邻居
- 更高的连接密度和路径冗余
- 适合研究网络收敛性能

使用方法:
    python3 generate_ospfv3_torus.py --size 10                    # 生成 10x10 单区域拓扑
    python3 generate_ospfv3_torus.py --size 40 --multi-area       # 生成 40x40 多区域拓扑
    python3 generate_ospfv3_torus.py --size 20 --area-size 10     # 每10x10划分一个区域
...
class OSPFv3TorusGenerator:
    """OSPFv3 Torus 拓扑生成器"""
    
    def __init__(self, size, source_dir="normal_test", multi_area=False, area_size=None, enable_bfd=False):
        self.size = size
        self.source_dir = source_dir
        self.target_dir = f"ospfv3_torus{size}x{size}_test"
        self.template_router = "router1"
        self.total_routers = size * size
        self.total_links = size * size * 2  # 每个路由器4个连接，每条链路计算一次
    ...
    def _check_system_requirements(self):
        ...
        print(f"=== {self.size}x{self.size} OSPFv3 Torus 拓扑信息 ===")
        print(f"节点数: {self.total_routers}")
        print(f"连接数: {self.total_links}")
        print(f"路由协议: OSPFv3")
        print(f"拓扑类型: Torus (环绕连接)")
    ...
    def get_neighbors(self, row, col):
        """计算给定位置的四个邻居坐标 (Torus 拓扑)"""
        return {
            'north': ((row - 1 + self.size) % self.size, col),
            'south': ((row + 1) % self.size, col),
            'west': (row, (col - 1 + self.size) % self.size),
            'east': (row, (col + 1) % self.size)
        }
...
# OSPFv3 Torus configuration for {hostname}
#
...
"""
    ...
    def generate_ospf6d_conf(self, row, col):
        ...
        node_type = "internal"  # Torus中所有节点都是内部节点
        neighbor_count = 4  # Torus中每个节点都有4个邻居

        # 获取接口地址
        interfaces = self.interface_addresses.get(router_name, {})

        config = f"""!
! OSPFv3 configuration for {hostname} in {self.size}x{self.size} Torus
...
"""
        ...
        for direction in ['north', 'south', 'west', 'east']:
            ...
            if intf_name in interfaces:
                n_row, n_col = neighbors[direction]
                ...
                if neighbor_intf in neighbor_interfaces:
                    ...
                    # 选择BFD profile - 与grid保持一致，都使用fast-convergence
                    profile = "fast-convergence"
...
"""
    ...
    def generate_clab_yaml(self):
        ...
        config = {
            "name": f"ospfv3-torus{self.size}x{self.size}",
            "mgmt": {
                "network": f"ospfv3_torus_mgmt_{self.size}x{self.size}",
                **mgmt_config
            },
            "topology": {
                "nodes": nodes,
        ...
        # 写入文件
        yaml_file = Path(self.target_dir) / f"ospfv3_torus{self.size}x{self.size}.clab.yaml"
    ...
    def print_summary(self):
        """打印摘要信息"""
        print(f"\n=== {self.size}x{self.size} OSPFv3 Torus 拓扑摘要 ===")
        ...
        # 打印拓扑统计
        print(f"\n=== Torus 拓扑节点分布 ===")
        print(f"内部节点: {self.total_routers} (4个邻居)")
        print(f"总节点数: {self.total_routers}")
        ...
        # 部署命令
        yaml_file = f"{self.target_dir}/ospfv3_torus{self.size}x{self.size}.clab.yaml"
        print(f"\n=== 部署命令 ===")
        print(f"部署: sudo containerlab deploy -t {yaml_file}")
        print(f"销毁: sudo containerlab destroy -t {yaml_file}")
        print(f"检查: sudo containerlab inspect -t ospfv3-torus{self.size}x{self.size}")
        print(f"连接: sudo docker exec -it clab-ospfv3-torus{self.size}x{self.size}-router_00_00 vtysh")
    ...
    def generate(self):
        ...
        print(f"开始生成 {self.size}x{self.size} OSPFv3 Torus 拓扑...")
...
def main():
    parser = argparse.ArgumentParser(
        description="OSPFv3 Torus 网络拓扑生成器 (支持最大4000节点)",
...
示例用法:
  ...
  %(prog)s --size 5 --yes                      # 跳过确认直接生成
    ...
    # 创建生成器
    try:
        generator = OSPFv3TorusGenerator(
            args.size,
            args.source,
            args.multi_area,
            args.area_size,
    ...
    if not args.yes:
        ...
        response = input(f"\n确认生成 {args.size}x{args.size} OSPFv3 Torus 拓扑? (y/N): ")
